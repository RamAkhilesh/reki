...
class CategoryCardWidget extends StatelessWidget {
    ...
    return Card(
      ...
      child: InkWell(
        ...
        child: Row(
          ...
          children: [
            // ── Accent bar ────────────────────────────────
            Container(width: 4, color: iconFg.withAlpha(180)),
  ...
  // Fixed vibrant accents for light mode — Material You tokens (primary,
  // secondary, etc.) are intentionally dark in light themes for contrast on
  // their own containers, but look near-black on neutral card surfaces.
  static const _lightAccents = <String, Color>{
    'movie': Color(0xFF3B82F6),
    'tv':    Color(0xFF22C55E),
    'anime': Color(0xFFA855F7),
    'manga': Color(0xFFEF4444),
    'game':  Color(0xFF06B6D4),
    'book':  Color(0xFFF59E0B),
  };

  static (Color, Color) _typeBadge(String type, ColorScheme cs) {
    final isLight = cs.brightness == Brightness.light;
    if (isLight) {
      final accent = _lightAccents[type] ?? cs.onSurfaceVariant;
      final bg = switch (type) {
        'movie'         => cs.primaryContainer,
        'tv' || 'book'  => cs.secondaryContainer,
        'anime'         => cs.tertiaryContainer,
        'manga'         => cs.errorContainer,
